...
export async function GET(_request: Request, { params }: { params: Promise<{ workspaceId: string }> }) {
  ...
  async function allRows(table: "core_files" | "core_tags" | "core_file_tags" | "core_notifications" | "core_audit_events") {
    ...
    for (let offset = 0; ; offset += 500) {
      ...
      if (table === "core_audit_events") query = query.eq("actor_id", userId);
  ...
  try {
    const [files, tags, fileTags, notifications, auditEvents] = await Promise.all([
    ...
    const payload = { format: "moduvita-core-v1", exported_at: new Date().toISOString(), workspace,
